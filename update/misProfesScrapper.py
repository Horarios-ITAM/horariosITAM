...
class MisProfesScrapper:
    ...
    def _urlProfe(self,nombre,apellido,id):
        return "https://www.misprofesores.com/profesores/"+self._fixStr(nombre)+"-"+self._fixStr(apellido)+"_"+id
    ...
    def scrap(self):
        ...
        profesores={p['n']+' '+p['a']:{'c':p['c'],'m':p['m'],'i':p['i'],'misProfesNom':p['n']+' '+p['a'],'link':self._urlProfe(p['n'],p['a'],p['i'])} for p in tempData}
    ...
    def match(self,profesores,matchRate):
        ...
        ratings={p:[float(d['c']),float(d['m'])] for p,d in matched.items() if d['c']}
        formatted={
            prof:{
                "link":links[prof],
        ...
        print("Se ligaron {} ({:.2f}%) profesores a evaluaciones de MisProfes".format(len(formatted),100*len(formatted)/len(profesores)))
        return formatted
