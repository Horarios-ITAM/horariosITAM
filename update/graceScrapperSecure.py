...
class GraceScrapperSecureArea:
    ...
    def login(self, usuario, passwd):
        ...
        """
        loginData = {"sid": usuario, "PIN": passwd}
        s = self.session
        s.get(loginUrl)
    ...
    def getClavesDeptos(self):
        ...
        """
        data = {"p_term": self.clavePeriodo, "p_calling_proc": "P_CrseSearch"}
    ...
    def getHTMLclase(self, depto, clave):
        ...
        """
        data = "term_in={}&sel_subj=dummy&sel_subj={}&SEL_CRSE={}&SEL_TITLE=&BEGIN_HH=0&BEGIN_MI=0&BEGIN_AP=a&SEL_DAY=dummy&SEL_PTRM=dummy&END_HH=0&END_MI=0&END_AP=a&SEL_CAMP=dummy&SEL_SCHD=dummy&SEL_SESS=dummy&SEL_INSTR=dummy&SEL_INSTR=%25&SEL_ATTR=dummy&SEL_ATTR=%25&SEL_LEVL=dummy&SEL_LEVL=%25&SEL_INSM=dummy&sel_dunt_code=&sel_dunt_unit=&call_value_in=&rsts=dummy&crn=dummy&path=1&SUB_BTN=View+Sections".format(
            self.clavePeriodo, depto, clave
        )
        ...
        for tr in table.find_all("tr")[2:]:
            ...
            for i, td in enumerate(tr.find_all("td")):
                ...
                if (
                    equiv
                    and temp[equiv] == ""
                    and equiv in ["depto", "clave", "grupo", "creditos", "nombre"]
                ):
    ...
    def formateaGrupo(self, grupo):
        ...
        try:
            ...
            out["nombre"] = (
                grupo["depto"]
                + "-"
                + grupo["clave"].replace("-LAB", "")
                + "-"
                + grupo["nombre"]
            )
            out["profesor"] = (
                grupo["profesor"].replace("/", " ").replace("(P)", "").strip()
            )
        ...
        self.profesores = list(
            set(
                [
                    ...
                    if len(grupo["profesor"].strip()) > 1
                ]
            )
        )

        self._print(
            "Se encontraron {} profesores en Grace!".format(len(self.profesores))
        )
